# This script creates training and test sets from the preprocessed data. 
# It reads the preprocessed JSON files, splits the data into training and test sets, and saves the splits as Parquet files.

# Input: Preprocessed JSON files from preprocessing.py.
# Output: Training and test sets in Parquet format, divided for application and traffic classification tasks.

# To execute: python create_train_test_set.py --source path/to/preprocessed_output --target path/to/train_test_output --test_size 0.2 --under_sampling True
...
def top_n_per_group(spark_df, groupby, topn):
    ...
    # Sample the top N rows per group based on a random seed
...
def split_train_test(df, test_size, under_sampling_train=True):
    ...
    # Split the data into training and test sets, with optional under-sampling 
    ...
    if under_sampling_train:
        ...
        min_label_count = min_label_count * 8
...
def create_train_test_for_task(df, label_col, test_size, under_sampling, data_dir_path):
    # Create train and test sets for a specific task (application or traffic classification)
...
def print_df_label_distribution(spark, path):
    # Print the distribution of labels in the DataFrame
    print(path)
    print(
